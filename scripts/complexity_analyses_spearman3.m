...
% 'calc_lz_complexity.m' by Quang Thai 
% 'Kolmogorov.m' by Stephen Faul 
%
...
%           'Stephen Faul (2025). Kolmogorov Complexity (https://www.mathworks.com/matlabcentral/fileexchange/6886-kolmogorov-complexity), MATLAB Central File Exchange. Retrieved May 15, 2025.'
...
% Calculate delta
delta_rho_lz = rho_lz - rho_lz_perc;

% Permutation test

for x = 1:5000
        %Create random vector
        rand_vec = rand(1, size(beta_comp_perc,2)) < 0.5;

        % Swap perception for imagery
        beta_comp_perc_rand = beta_comp_perc;
        beta_comp_perc_rand(rand_vec) = beta_comp_imagery(rand_vec);
    
        % Swap imagery for perception
        beta_comp_imagery_rand = beta_comp_imagery;
        beta_comp_imagery_rand(rand_vec) = beta_comp_perc(rand_vec);
    
        % Spearman Correlation
        rho1_perm = corr(vviq, beta_comp_perc_rand', 'Type', 'Spearman');
        rho2_perm = corr(vviq, beta_comp_imagery_rand', 'Type', 'Spearman');
    
        % Perm deltas
        perm_delta_lz(x) = rho2_perm - rho1_perm;
end

% P-value
 pval_perm_delta_lz = sum(perm_delta_lz >= delta_rho_lz)/5000;
...
for x = 1:5000
        ...
        % Perm deltas
        perm_delta_k(x) = rho2_perm - rho1_perm;
end

% P-value
 pval_perm_delta_k = sum(perm_delta_k >= delta_rho_k)/5000;