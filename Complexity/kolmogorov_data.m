% Calculates the Kolmogorov Complexity of TS.
%
%% NB:
% Requires 'Kolmogorov.m' by Stephen Faul in order to work!
%
% Citation: 'Stephen Faul (2025). Kolmogorov Complexity (https://www.mathworks.com/matlabcentral/fileexchange/6886-kolmogorov-complexity), MATLAB Central File Exchange. Retrieved May 15, 2025.'
...
% cd '/Volumes/research-data/PRJ-shine_hpc/Aphantasia_fMRI'
    ...
    load All_dsmtx.mat
    load subj_id.mat % 1:23 -> aphan; 23:43 -> ctrl;
    load idx_ac.mat
...
%PLACES    -------------------------------------------------------------------------
    ...
    for ID = 1:nSubs
        ...
        bold_bin = bold.ts'>0;
        for tt = 1:size(bold_bin,2)
        temp_comp(tt,ID) = kolmogorov(flatten_mat(bold_bin(temp_lobe_mask==1,tt)));% Column corresponds to sub_id
        end
...
%% 1:23 --> aphantasics; 24:end --> controls
    ...
    delta_k_faces = beta_comp_faces(3,:) - beta_comp_faces(2,:);
    delta_k_places = beta_comp_places(3,:) - beta_comp_places(2,:);
...
% -------------------------------------------------------------------------------------------------------------------
    ...
    aphan_k = horzcat(delta_k_faces(1,1:23),delta_k_places(1,1:23));
    ctrl_k = horzcat(delta_k_faces(1,24:end),delta_k_places(1,24:end));
    %% 1:23 --> aphantasics; 24:end --> controls
    
    size1 = size(aphan_k,2);
    size2 = size(ctrl_k,2);
    
    null_delta = zeros(iter,1);
    
    data_combo = horzcat(aphan_k,ctrl_k);
